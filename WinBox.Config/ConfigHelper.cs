...
namespace WinBox.Config
{
    public static class ConfigHelper
    {
        ...
        public static T[] GetEnumValues<T>() => Enum.GetValues(typeof(T)).OfType<T>().ToArray();
        ...
        public static IDictionary<string, string> Normalize(IDictionary dict, string prefix = null) => ToStringDict(dict)
    .Select(e => new { Name = prefix + ToCamelCase(e.Key), e.Value })
    .ToDictionary(k => k.Name, v => v.Value);

        static string ToCamelCase(string text) => text.Substring(0, 1).ToUpperInvariant()
    + text.Substring(1).ToLowerInvariant();
        ...
        public static IDictionary<string, object> ReflectObjToDict<T>(T obj,
                                                          string prefix = null) => ReflectToDict(typeof(T), obj, prefix: prefix);
        ...
        static string ToConfigString(object obj)
        {
            var type = obj.GetType();
            switch (type.FullName)
            {
